...
def test_calculate_income_tax_basic_rate_total(): 
    # basic rate is 20% for income between 12570 and 50270
    assert round(calculate_income_tax(12570.01)['total_tax'], 3) == 0.002

    assert calculate_income_tax(12571)['total_tax'] == 0.20
    assert calculate_income_tax(12571, 1000)['total_tax'] == 0.20
    assert round(calculate_income_tax(12571, 6999)['total_tax'], 2) == 200.20 #0.20 * (12571-12570) + 0.20 * (6999-4999)

    assert calculate_income_tax(15000)['total_tax'] == 486 #0.20 * (15000 - 12570)
    assert calculate_income_tax(15000, 1000)['total_tax'] == 486 #0.20 * (15000 - 12570)
    assert calculate_income_tax(15000, 2000)['total_tax'] == 486 #0.20 * (15000 - 12570)
    assert calculate_income_tax(15000, 4000)['total_tax'] == 572 #0.20 * (15000 - 12570) + 86

    assert calculate_income_tax(20000, 1000)['total_tax'] == 1486 #0.20 * (20000 - 12570)
    assert calculate_income_tax(20000, 2000)['total_tax'] == 1686 #0.20 * (20000 - 12570) + 200

    assert calculate_income_tax(50270)['total_tax'] == 7540 #0.20 * (50270 - 12570)

def test_calculate_income_tax_higher_rate_total():
    # higher rate is 40% for income between 50270 and 125140
    assert round(calculate_income_tax(50270.01)['total_tax'], 3) == 7540.004 #0.40 * (50270.01 - 50270) + 0.20 * (50270 - 12570)
    assert calculate_income_tax(50271)['total_tax'] == 7540.40 #0.40 * (50271 - 50270) + 0.20 * (50270 - 12570)
    assert calculate_income_tax(60000)['total_tax'] == 11432 #0.40 * (60000 - 50270) + 0.20 * (50270 - 12570)
    assert calculate_income_tax(125140)['total_tax'] == 37488 #0.40 * (125140 - 50270) + 0.20 * (50270 - 12570)

def test_calculate_income_tax_additional_rate_total():
    # additional rate is 45% for income above 125140
    assert calculate_income_tax(125141)['total_tax'] == 37488.45 #0.45 * (125141 - 125140) + 0.40 * (125140 - 50270) + 0.20 * (50270 - 12570)
    assert calculate_income_tax(150000)['total_tax'] == 48675 #0.45 * (150000 - 125140) + 0.40 * (125140 - 50270) + 0.20 * (50270 - 12570)
    assert calculate_income_tax(200000)['total_tax'] == 71175 #0.45 * (200000 - 125140) + 0.40 * (125140 - 50270) + 0.20 * (50270 - 12570)
...
def test_calculate_income_tax_saving_interest_0():
    # any non-saving income with no saving income should return 0 tax from saving interest
    assert calculate_income_tax(1000, 0)['tax_saving_interest'] == 0
    assert calculate_income_tax(12570, 0)['tax_saving_interest'] == 0
    assert calculate_income_tax(50270, 0)['tax_saving_interest'] == 0
    assert calculate_income_tax(125140, 0)['tax_saving_interest'] == 0

    # anyone does not exceed their pa and psa should return 0 tax from saving interest
    assert calculate_income_tax(0, 17570)['tax_saving_interest'] == 0
    assert calculate_income_tax(1000, 17570)['tax_saving_interest'] == 0
    assert calculate_income_tax(12569, 5000)['tax_saving_interest'] == 0 
    assert calculate_income_tax(12569, 6000)['tax_saving_interest'] == 0
    assert calculate_income_tax(12570, 1000)['tax_saving_interest'] == 0
    assert calculate_income_tax(49269, 1000)['tax_saving_interest'] == 0
    assert calculate_income_tax(50270, 500)['tax_saving_interest'] == 0
    assert calculate_income_tax(124639, 500)['tax_saving_interest'] == 0

def test_calculate_income_tax_saving_interest():
    # test anyone who exceed their pa and psa
    assert calculate_income_tax(12570, 7000)['tax_saving_interest'] == 200 # (7000 - 6000) * 0.20
    assert calculate_income_tax(35000, 2000)['tax_saving_interest'] == 200 # (2000 - 1000) * 0.20
    assert calculate_income_tax(50270, 1500)['tax_saving_interest'] == 400 # (1500 - 500) * 0.40

def test_calculate_income_tax_saving_interest_additional_rate():
    # additional rate has no psa, so it should just be the saving * tax rate
    assert calculate_income_tax(126141, 1000)['tax_saving_interest'] == 450 #1000*0.45
    assert calculate_income_tax(124640, 1000)['tax_saving_interest'] == 425 #500*0.25 + 500*0.45

def test_calculate_income_tax_logical(): 
    # test if the values inside the disctionary added up correctly
    assert calculate_income_tax(12570, 7000)['total_tax'] == calculate_income_tax(12570, 7000)['tax_saving_interest'] + calculate_income_tax(12570, 7000)['tax_non_saving_income']
    assert calculate_income_tax(35000, 2000)['total_tax'] == calculate_income_tax(35000, 2000)['tax_saving_interest'] + calculate_income_tax(35000, 2000)['tax_non_saving_income']
    assert calculate_income_tax(50270, 1500)['total_tax'] == calculate_income_tax(50270, 1500)['tax_saving_interest'] + calculate_income_tax(50270, 1500)['tax_non_saving_income']
...
def test_scenario_1(): 
    ...
    '''
    assert calculate_income_tax(14500, 2500)['total_tax'] == 386
    assert calculate_income_tax(14500, 2500)['tax_non_saving_income'] == 386
    assert calculate_income_tax(14500, 2500)['tax_saving_interest'] == 0
...
def test_tax_from_saving_interest_scenario_2(): 
    ...
    '''
    assert calculate_income_tax(0, 20000)['total_tax'] == 286
    assert calculate_income_tax(0, 20000)['tax_non_saving_income'] == 0
    assert calculate_income_tax(0, 20000)['tax_saving_interest'] == 286
...
def test_tax_from_saving_interest_scenario_3(): 
    ...
    '''
    assert calculate_income_tax(35000, 5000)['tax_saving_interest'] == 800
...
def test_tax_from_saving_interest_scenario_4(): 
    ...
    '''
    assert calculate_income_tax(49270, 990)['total_tax'] == 7340
    assert calculate_income_tax(49270, 990)['tax_non_saving_income'] == 7340
    assert calculate_income_tax(49270, 990)['tax_saving_interest'] == 0
...
def test_tax_from_saving_interest_scenario_5(): 
    ...
    In this scenario all of your savings interest would be tax-free.
    '''
    assert calculate_income_tax(49270, 1010)['total_tax'] == 7444 # 7340 + 104
    assert calculate_income_tax(49270, 1010)['tax_non_saving_income'] == 7340
    assert calculate_income_tax(49270, 1010)['tax_saving_interest'] == 104 # 500 tax-free, 500 taxed at 20%, 10 taxed at 40%